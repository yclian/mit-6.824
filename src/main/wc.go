...
func Map(value string) *list.List {
	...
	nonWordsRe := regexp.MustCompile("[^a-zA-Z]")

	for _, word := range strings.Fields(value) {

		wordOnly := nonWordsRe.ReplaceAllString(word, "")
		if wordOnly == "" {
			continue
		}

		if v, ok := mapping[wordOnly]; ok {
			mapping[wordOnly] = v + 1
		} else {
			mapping[wordOnly] = 1
		}
	}
	...
	for k, v := range mapping {
		out.PushBack(mapreduce.KeyValue{k, strconv.Itoa(v)})
	}
...
func Reduce(key string, values *list.List) string {
	...
	for v := values.Front(); v != nil; v = v.Next() {
		if i, e := strconv.Atoi(v.Value.(string)); e == nil {
			count += i
		}
	}
